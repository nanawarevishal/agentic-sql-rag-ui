...
function errorRateSeverity(fraction: number): "good" | "warning" | "critical" {
  if (fraction >= 0.05) return "critical";
  if (fraction >= 0.02) return "warning";
  return "good";
}
          ...
          {isFetching && !data ? (
            ...
            <>
              <div className="admin-usage-stat-groups">
                <div className="admin-usage-stat-group">
                  <span className="admin-usage-group-label">Volume</span>
                  <div className="admin-usage-stats">
                    ...
                    <div className={`admin-usage-stat is-${errorRateSeverity(data.error_rate)}`}>
                ...
                <div className="admin-usage-stat-group">
                  <span className="admin-usage-group-label">Latency</span>
                  <div className="admin-usage-stats">
                    <div className="admin-usage-stat">
                      <span className="admin-usage-stat-label">Avg</span>
                      <span className="admin-usage-stat-value">{formatMs(data.avg_duration_ms)}</span>
                    </div>
                    <div className="admin-usage-stat">
                      <span className="admin-usage-stat-label">p50</span>
                      <span className="admin-usage-stat-value">{formatMs(data.p50_duration_ms)}</span>
                    </div>
                    <div className="admin-usage-stat">
                      <span className="admin-usage-stat-label">p95</span>
                ...
                <div className="admin-usage-stat-group">
                  <span className="admin-usage-group-label">Cost (est.)</span>
                  <div className="admin-usage-stats">
                    <div className="admin-usage-stat">
                      <span className="admin-usage-stat-label">Total</span>
                      <span className="admin-usage-stat-value">{formatCost(data.total_cost_usd)}</span>
                    </div>
                    <div className="admin-usage-stat">
                      <span className="admin-usage-stat-label">Avg / query</span>
                  ...
                  <table className="trace-rows-table admin-usage-table">
                    ...
                    <tbody>
                      {data.per_user.map((u) => (
                        <tr key={u.user_id}>
                          <td>{u.email}</td>
                          <td>{u.query_count.toLocaleString()}</td>
                          <td className={u.error_count > 0 ? "admin-usage-error-count" : undefined}>
                            {u.error_count}
                          </td>
